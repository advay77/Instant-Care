...
function Signin() {
  ...
  const handleGoogleSuccess = (credentialResponse) => {
    ...
    try {
      ...
      // Store token (in real app, send to backend for verification)
      localStorage.setItem('authToken', credential);
      setSuccess('Signed in with Google!');
      // Optionally decode JWT for user info (for demo only)
                  ...
                  {authMethod === 'google' && (
                    <div className="mb-4 text-center">
                      <GoogleLogin
                        ...
                        useOneTap
                      />
                      {loading && <div className="mt-3">Signing in...</div>}
                      {googleUser && (
                        <div className="mt-3 text-center">
                          <img src={googleUser.picture} alt="Profile" style={{width: 48, borderRadius: '50%', marginBottom: 8}} />
                          <div style={{fontWeight: 600}}>{googleUser.name}</div>
                          <div style={{fontSize: '0.95rem', color: '#666'}}>{googleUser.email}</div>
                        </div>
                      )}
                    </div>
                  )}
